...
function LoginScreen( { handleSubmit,startLogin, isLoading, submitting, isAuthenticated, user, loginError, clearLoginError }) {
  useEffect(() => {
    if(isAuthenticated && user)
      history.replace("/home_screen_super_admin");
    if(loginError)
      setTimeout(clearLoginError, 4000);
  },[isAuthenticated, user, loginError, clearLoginError]);
  return (
      ...
      { loginError &&
       <div className="absolute w-full flex justify-center" role="alert">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 w-11/12 rounded relative">
          ...
          <span className="absolute top-0 bottom-0 right-0 px-4 py-3" onClick={clearLoginError}>
        ...
        <div className="flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 rounded-md w-2/6">
          <div className="max-w-full w-full space-y-8">
            ...
            <form onSubmit={handleSubmit(startLogin)} className="mt-8 space-y-6" >
              ...
              <div>
                <button disabled={submitting} type="submit" className="group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-primary hover:bg-secondary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-secondary">
                  Iniciar Sesión
...
export default connect(
  state => ({
    loginError: selectors.getAuthenticatingError(state),
  ...
  dispatch => ({
    ...
    clearLoginError(values) {
      dispatch(actionsAuth.clearLoginError(values));
    },
  }),
)(reduxForm({ 
  form: 'login',
